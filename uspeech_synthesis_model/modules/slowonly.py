...
class Slowonly(nn.Module):
    def __init__(self, embed_dim, depth=50):
        super().__init__()
        self.embed_dim = embed_dim
        self.depth = depth
        self.video_project_head = nn.Linear(2048, embed_dim)
        self.resnet3dslowonly = ResNet3dSlowOnly(depth=self.depth, pretrained=None)
    def forward(self, x):
        x = self.resnet3dslowonly(x)
        x = x.squeeze(3).squeeze(3).permute(0, 2, 1)
        x = self.video_project_head(x)
        return x